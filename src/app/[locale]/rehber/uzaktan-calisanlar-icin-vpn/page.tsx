...
import { getRemoteWorkersContent } from "@/content/guides/vpn-for-remote-workers";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const c = getRemoteWorkersContent(locale);
  const url = absoluteUrl(
    getLocalizedPath({
      locale: asAppLocale(locale),
      section: "guide",
      contentId: "vpn-for-remote-workers",
    }),
  );
  return {
    title: c.metaTitle,
    ...
    alternates: contentAlternates("vpn-for-remote-workers", locale),
    openGraph: {
      title: c.ogTitle,
...
export default async function Page({ params }: Props) {
  ...
  const cardIcons = [Coffee, Lock, Globe];
    ...
    <>
      ...
      <Container size="md" className="py-12 sm:py-16">
        ...
        <header className="mt-6">
          <Badge variant="brand">
            <Briefcase className="size-3" /> {c.badge}
          </Badge>
        ...
        <article className="mt-16 prose prose-stone max-w-none">
          ...
          <h3>{c.dataAtRisk.h3}</h3>
          <ul>
            {c.dataAtRisk.items.map((item) => (
              <li key={item.bold}>
          ...
          <h2>{c.corpVsCommercial.h2}</h2>
          <table>
            <thead>
              <tr>
                {c.corpVsCommercial.headers.map((h) => (
                  <th key={h}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {c.corpVsCommercial.rows.map((row) => (
                <tr key={row[0]}>
                  {row.map((cell, i) => (
                    <td key={i}>{cell}</td>
                  ))}
          ...
          <p>
            <strong>{c.corpVsCommercial.adviceBold}</strong>
            {c.corpVsCommercial.adviceText}
          </p>

          <h2>{c.nomadChecklist.h2}</h2>
          <ul>
            {c.nomadChecklist.items.map((item) => (
              <li key={item.bold}>
                <strong>{item.bold}</strong>
                {item.text}
              </li>
          ...
          <h3>{c.scenarios.travel.h3}</h3>
          <p>{c.scenarios.travel.p}</p>

          <h3>{c.scenarios.privacy.h3}</h3>
          <p>
            <Link href={c.scenarios.privacy.link1.href}>
              {c.scenarios.privacy.link1.text}
            </Link>
            {c.scenarios.privacy.sep}
            <Link href={c.scenarios.privacy.link2.href}>
              {c.scenarios.privacy.link2.text}
            </Link>
            {c.scenarios.privacy.after}
          </p>

          <h3>{c.scenarios.budget.h3}</h3>
          <p>
            <Link href={c.scenarios.budget.link.href}>
              {c.scenarios.budget.link.text}
            </Link>
            {c.scenarios.budget.after}
          </p>
