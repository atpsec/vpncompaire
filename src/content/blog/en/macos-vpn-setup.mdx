...
On Apple silicon, native ARM builds can improve compatibility and efficiency. Check current architecture support in provider release notes; the effect varies by app, protocol and workload.
...
Native ARM64 clients can be more efficient on Apple silicon, but the effect depends on the application, protocol, radio conditions and workload; we do not publish a universal multiplier.

WireGuard numbers:
- M2 Air, native app: 800-950 Mbps
- Older apps running through Rosetta may use more resources.
- Native ARM builds can be more efficient depending on the device and app version; there is no fixed Mbps or percentage guarantee.
