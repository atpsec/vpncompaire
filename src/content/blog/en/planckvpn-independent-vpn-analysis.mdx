...
That aligns with **ownership transparency**, which VPN Advisor also cares about. The difference: we already list options such as [Mullvad](/en/inceleme/mullvad) with no affiliate tie and years of evidence. PlanckVPN has not reached that evidence level yet.
...
That split is common in the industry — "no tunnel logs" is not the same as "no metadata at all". [Mullvad](/en/inceleme/mullvad) and [Proton VPN](/en/inceleme/proton-vpn) back their claims with repeated third-party audits. PlanckVPN has **no public independent no-log audit** yet.
...
We applied our [six criteria](/en/metodoloji) using **public information only** — no hands-on testing:

### 1. Privacy policy and jurisdiction

- **Plus:** Readable policy; AdMob funding disclosed.
- **Minus:** Virginia, USA — [Five Eyes](/en/sozluk) context applies. Less favourable than Panama (NordVPN) or Sweden/Mullvad for jurisdiction-sensitive users.
...
- Anyone requiring audit reports — [Proton VPN](/en/inceleme/proton-vpn) or [Mullvad](/en/inceleme/mullvad) are more mature
...
PlanckVPN is on our **watchlist**. A full provider profile (`/inceleme/planckvpn`) would need several of:
...
- [Mullvad review](/en/inceleme/mullvad) — no affiliate, privacy-first reference
- [Our methodology](/en/metodoloji)
