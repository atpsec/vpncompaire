...
import { Award, Check, ExternalLink, MapPin, Server, Smartphone, X, FileSearch } from "lucide-react";
...
export function ComparePicker() {
  ...
  const toggle = (slug: string) => setSelected((prev) => prev.includes(slug) ? (prev.length <= MIN ? prev : prev.filter((s) => s !== slug)) : (prev.length >= MAX ? prev : [...prev, slug]));
  ...
  return <section className="py-16 sm:py-20 bg-surface-subtle/40 border-y border-border">
    <Container>
      <div className="mb-8 max-w-2xl"><span className="text-[11px] font-semibold uppercase tracking-[0.18em] text-brand-700">{t("kicker")}</span><h2 className="mt-2 text-3xl sm:text-4xl font-bold tracking-tight text-ink-strong">{t("h2")}</h2><p className="mt-3 text-ink-muted">{t("intro")}</p></div>
...
function ProductColumn({ product, locale }: { product: Product; locale: Locale }) {
  ...
  return <Card className="flex flex-col h-full">
    <div className="p-4 border-b border-border"><div className="flex items-center gap-3"><VPNLogo slug={product.slug} size={44} /><div className="min-w-0"><h3 className="font-bold text-ink-strong leading-tight">{product.brand}</h3><p className="text-xs text-ink-subtle line-clamp-1">{product.positioning}</p></div></div><div className="mt-3"><div className="flex items-center gap-1.5 text-[10px] uppercase tracking-wider text-ink-subtle font-semibold"><FileSearch className="size-3" /> {copy.sourceStatus}</div><div className="mt-1 text-sm font-semibold text-brand-700 line-clamp-2">{product.highlights.audits ?? "—"}</div></div></div>
    ...
    <div className="p-4 border-t border-border bg-surface-subtle/30 rounded-b-xl"><div className="text-[10px] uppercase tracking-wider text-ink-subtle font-semibold">{t("bestValueLabel")}</div><div className="mt-1 text-sm font-medium text-ink-strong line-clamp-1">{bestPlan.name}</div><div className="mt-1 flex items-baseline gap-1"><span className="text-xl font-bold text-ink-strong tabular-nums">${bestPlan.monthlyPriceUsd.toFixed(2)}</span><span className="text-xs text-ink-subtle">{t("perMonth")}</span></div>{monthlyPlan && monthlyPlan !== bestPlan && <div className="mt-1 text-[11px] text-ink-faint">{t("monthlyLabel")} ${monthlyPlan.monthlyPriceUsd.toFixed(2)}</div>}<div className="mt-3 flex flex-col gap-1.5"><Button asChild variant="primary" size="sm"><a href={product.pricingUrl} rel="noopener nofollow" target="_blank">{copy.officialSource}<ExternalLink className="size-3.5" /></a></Button><Button asChild variant="ghost" size="sm"><Link href={`/inceleme/${product.slug}`}>{copy.profileLink}</Link></Button></div></div>
  </Card>;
}
